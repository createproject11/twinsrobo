<?= $this->extend('/templates/super_user'); ?>

<?= $this->section('content'); ?>
<!-- Pencarian -->
<div id="pencarian">
    <!-- Awal Container -->
    <div class="container">
        <div class="row">
            <div class="col" data-aos="fade-right">
                <h1>Ada Apa di Twins Robo</h1>
                <p>
                    Cerita mitra,berita seru, dan promo terbaru. Baca semua artikel
                    soal Twins Robo di sini.
                </p>

                <form class="form-inline">
                    <input class="form-control mr-md-2" type="search" placeholder="Search Blog Post..."
                        aria-label="Search" />
                    <button class="btn btn-primary my-2 my-sm-0" type="submit">
                        <i class="fas fa-search" style="font-weight: 200"></i>
                        Cari
                    </button>
                </form>

                <img src="/public/img/kategori1.png" alt="">
            </div>
        </div>
    </div>
    <!-- Akhir Container -->
</div>
<!-- Akhir Pencarian -->

<!-- Awal Blog -->
<div id="blog">
<<<<<<< HEAD
    <!--Awal Blog Bagian Atas -->
    <div class="blogAtas">
        <div class="container">
            <div class="row">
                <!-- Awal atasKiri -->
                <div class="atasKiri col-lg-6">
                    <div class="card">
                        <div class="gambar" data-aos="fade-up" data-aos-delay="300">
                            <img src="img/dashboard.png" class="card-img-top" alt="" style="width: 100%;" />
                        </div>
                        <div class="card-body aos-init aos-animate" data-aos="fade-right">
                            <div class="isiSub">
                                <img src="img/Logo Apps Twins Robo.png" alt="" />
                                <h4 class="sub-title">Twins Robo App</h4>
                            </div>
                            <h2 class="title">
                                Fitur Info Lomba Robot Terbaru, Dengan Banyaknya Lomba-Lomba
                            </h2>
                            <p class="date">Twins Robo / 3 Maret 2021</p>
                        </div>
                    </div>
                </div>
                <!-- Akhir atasKiri -->

                <!-- Awal atasKanan -->
                <div class="atasKanan col-lg-6" data-aos="fade-up" data-aos-delay="300">
                    <div class="row">
                        <div class="col-lg-6 col-md-6">
                            <div class="card">
                                <div class="gambar">
                                    <img src="img/dashboard.png" class="card-img-top" alt=""
                                        style="width: 171px; height: 128px" />
                                </div>
                                <div class="card-body">
                                    <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                                    <p class="date">Twins Robo / 3 Maret 2021</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-6 col-md-6">
                            <div class="card">
                                <div class="gambar">
                                    <img src="img/dashboard.png" class="card-img-top" alt=""
                                        style="width: 171px; height: 128px" />
                                </div>
                                <div class="card-body">
                                    <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                                    <p class="date">Twins Robo / 3 Maret 2021</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-6 col-md-6">
                            <div class="card">
                                <div class="gambar">
                                    <img src="img/dashboard.png" class="card-img-top" alt=""
                                        style="width: 171px; height: 128px" />
                                </div>
                                <div class="card-body">
                                    <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                                    <p class="date">Twins Robo / 3 Maret 2021</p>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-6 col-md-6">
                            <div class="card">
                                <div class="gambar">
                                    <img src="img/dashboard.png" class="card-img-top" alt=""
                                        style="width: 171px; height: 128px" />
                                </div>
                                <div class="card-body">
                                    <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                                    <p class="date">Twins Robo / 3 Maret 2021</p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <!-- Akhir atasKanan -->
=======
  <!--Awal Blog Bagian Atas -->
  <div class="blogAtas">
    <div class="container">
      <div class="row">
        <!-- Awal atasKiri -->
        <div class="atasKiri col-lg-6">
          <div class="card">
            <div class="gambar">
              <img src="img/dashboard.png" class="card-img-top" alt="" style="width: 100%;" />
            </div>
            <div class="card-body">
              <div class="isiSub">
                <img src="img/Logo Apps Twins Robo.png" alt="" />
                <h4 class="sub-title">Twins Robo App</h4>
              </div>
              <a href="">
                <h2 class="title">
                  Fitur Info Lomba Robot Terbaru, Dengan Banyaknya Lomba-Lomba
                </h2>
              </a>
              <p class="date">Twins Robo / 3 Maret 2021</p>
>>>>>>> 08363230f109d847aea069304296168905e1a1f6
            </div>
        </div>
    </div>
    <!--Akhir Blog Bagian Atas -->

<<<<<<< HEAD
    <!--Awal Blog Bagian Bawah -->
    <div class="blogBawah">
        <div class="container">
            <div class="row aos-init aos-animate" data-aos="fade-up" data-aos-delay="200">
                <div class="col-lg-4 ">
                    <div class="card">
                        <div class="gambar">
                            <img src="img/Dashboard.png" class="card-img-top" style="width: fit-content;" alt="" />
                        </div>
                        <div class="card-body">
                            <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                            <p class="date">Twins Robo / 3 Maret 2021</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 ">
                    <div class="card">
                        <div class="gambar">
                            <img src="img/dashboard.png" class="card-img-top" alt="" style="width: fit-content;" />
                        </div>
                        <div class="card-body">
                            <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                            <p class="date">Twins Robo / 3 Maret 2021</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="card">
                        <div class="gambar">
                            <img src="img/dashboard.png" class="card-img-top" alt="" style="width: fit-content;" />
                        </div>
                        <div class="card-body">
                            <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                            <p class="date">Twins Robo / 3 Maret 2021</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="card">
                        <div class="gambar">
                            <img src="img/dashboard.png" class="card-img-top" alt="" style="width: fit-content;" />
                        </div>
                        <div class="card-body">
                            <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                            <p class="date">Twins Robo / 3 Maret 2021</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="card">
                        <div class="gambar">
                            <img src="img/dashboard.png" class="card-img-top" alt="" style="width: fit-content;" />
                        </div>
                        <div class="card-body">
                            <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                            <p class="date">Twins Robo / 3 Maret 2021</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="card">
                        <div class="gambar">
                            <img src="img/dashboard.png" class="card-img-top" alt="" style="width: fit-content;" />
                        </div>
                        <div class="card-body">
                            <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                            <p class="date">Twins Robo / 3 Maret 2021</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="card">
                        <div class="gambar">
                            <img src="img/dashboard.png" class="card-img-top" alt="" style="width: fit-content;" />
                        </div>
                        <div class="card-body">
                            <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                            <p class="date">Twins Robo / 3 Maret 2021</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="card">
                        <div class="gambar">
                            <img src="img/dashboard.png" class="card-img-top" alt="" style="width: fit-content;" />
                        </div>
                        <div class="card-body">
                            <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                            <p class="date">Twins Robo / 3 Maret 2021</p>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4">
                    <div class="card">
                        <div class="gambar">
                            <img src="img/dashboard.png" class="card-img-top" alt="" style="width: fit-content;" />
                        </div>
                        <div class="card-body">
                            <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                            <p class="date">Twins Robo / 3 Maret 2021</p>
                        </div>
                    </div>
                </div>
=======
        <!-- Awal atasKanan -->
        <div class="atasKanan col-lg-6">
          <div class="row">
            <div class="col-lg-6 col-md-6">
              <a href="">
                <div class="card">
                  <div class="gambar">
                    <img src="img/dashboard.png" class="card-img-top" alt="" style="width: 171px; height: 128px" />
                  </div>
                  <div class="card-body">
                    <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                    <p class="date">Twins Robo / 3 Maret 2021</p>
                  </div>
                </div>
              </a>
            </div>
            <div class="col-lg-6 col-md-6">
              <a href="">
                <div class="card">
                  <div class="gambar">
                    <img src="img/dashboard.png" class="card-img-top" alt="" style="width: 171px; height: 128px" />
                  </div>
                  <div class="card-body">
                    <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                    <p class="date">Twins Robo / 3 Maret 2021</p>
                  </div>
                </div>
              </a>
            </div>
          </div>
          <div class="row">
            <div class="col-lg-6 col-md-6">
              <a href="">
                <div class="card">
                  <div class="gambar">
                    <img src="img/dashboard.png" class="card-img-top" alt="" style="width: 171px; height: 128px" />
                  </div>
                  <div class="card-body">
                    <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                    <p class="date">Twins Robo / 3 Maret 2021</p>
                  </div>
                </div>
              </a>
            </div>
            <div class="col-lg-6 col-md-6">
              <a href="">
                <div class="card">
                  <div class="gambar">
                    <img src="img/dashboard.png" class="card-img-top" alt="" style="width: 171px; height: 128px" />
                  </div>
                  <div class="card-body">
                    <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                    <p class="date">Twins Robo / 3 Maret 2021</p>
                  </div>
                </div>
              </a>
>>>>>>> 08363230f109d847aea069304296168905e1a1f6
            </div>

<<<<<<< HEAD
            <!-- Awal Pagination -->
            <ul class="pagination">
                <li class="page-item">
                    <a class="page-link" href="#" aria-label="First">
                        <span aria-hidden="true"> <img src="img/next.png" alt=""
                                style="transform: rotate(180deg) scale(.6)"> </span>
                    </a>
                </li>
                <li class="page-item">
                    <a class="page-link" href="#" aria-label="Previous">
                        <span aria-hidden="true"> <i class="bi bi-arrow-left"></i> </span>
                    </a>
                </li>
                <li class="page-item"><a class="page-link" href="#">1</a></li>
                <li class="page-item"><a class="page-link" href="#">2</a></li>
                <li class="page-item"><a class="page-link" href="#">3</a></li>
                <li class="page-item">
                    <a class="page-link" href="#" aria-label="Next">
                        <span aria-hidden="true"> <i class="bi bi-arrow-right"></i> </span>
                    </a>
                </li>
                <li class="page-item">
                    <a class="page-link" href="#" aria-label="Last">
                        <span aria-hidden="true"> <img src="img/next.png" alt="" style="transform: scale(.6);"> </span>
                    </a>
                </li>
            </ul>
            <!-- Akhir Pagination -->
        </div>
=======
  <!--Awal Blog Bagian Bawah -->
  <div class="blogBawah">
    <div class="container">
      <div class="row">
        <div class="col-lg-4 ">
          <a href="">
            <div class="card">
              <div class="gambar">
                <img src="img/Dashboard.png" class="card-img-top" style="width: fit-content;" alt="" />
              </div>
              <div class="card-body">
                <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                <p class="date">Twins Robo / 3 Maret 2021</p>
              </div>
            </div>
          </a>
        </div>
        <div class="col-lg-4 ">
          <a href="">
            <div class="card">
              <div class="gambar">
                <img src="img/dashboard.png" class="card-img-top" alt="" style="width: fit-content;" />
              </div>
              <div class="card-body">
                <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                <p class="date">Twins Robo / 3 Maret 2021</p>
              </div>
            </div>
          </a>
        </div>
        <div class="col-lg-4">
          <a href="">
            <div class="card">
              <div class="gambar">
                <img src="img/dashboard.png" class="card-img-top" alt="" style="width: fit-content;" />
              </div>
              <div class="card-body">
                <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                <p class="date">Twins Robo / 3 Maret 2021</p>
              </div>
            </div>
          </a>
        </div>
      </div>
      <div class="row">
        <div class="col-lg-4 ">
          <a href="">
            <div class="card">
              <div class="gambar">
                <img src="img/Dashboard.png" class="card-img-top" style="width: fit-content;" alt="" />
              </div>
              <div class="card-body">
                <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                <p class="date">Twins Robo / 3 Maret 2021</p>
              </div>
            </div>
          </a>
        </div>
        <div class="col-lg-4 ">
          <a href="">
            <div class="card">
              <div class="gambar">
                <img src="img/dashboard.png" class="card-img-top" alt="" style="width: fit-content;" />
              </div>
              <div class="card-body">
                <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                <p class="date">Twins Robo / 3 Maret 2021</p>
              </div>
            </div>
          </a>
        </div>
        <div class="col-lg-4">
          <a href="">
            <div class="card">
              <div class="gambar">
                <img src="img/dashboard.png" class="card-img-top" alt="" style="width: fit-content;" />
              </div>
              <div class="card-body">
                <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                <p class="date">Twins Robo / 3 Maret 2021</p>
              </div>
            </div>
          </a>
        </div>
      </div>
      <div class="row">
        <div class="col-lg-4 ">
          <a href="">
            <div class="card">
              <div class="gambar">
                <img src="img/Dashboard.png" class="card-img-top" style="width: fit-content;" alt="" />
              </div>
              <div class="card-body">
                <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                <p class="date">Twins Robo / 3 Maret 2021</p>
              </div>
            </div>
          </a>
        </div>
        <div class="col-lg-4 ">
          <a href="">
            <div class="card">
              <div class="gambar">
                <img src="img/dashboard.png" class="card-img-top" alt="" style="width: fit-content;" />
              </div>
              <div class="card-body">
                <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                <p class="date">Twins Robo / 3 Maret 2021</p>
              </div>
            </div>
          </a>
        </div>
        <div class="col-lg-4">
          <a href="">
            <div class="card">
              <div class="gambar">
                <img src="img/dashboard.png" class="card-img-top" alt="" style="width: fit-content;" />
              </div>
              <div class="card-body">
                <h2 class="title">Lorem ipsum dolor sit amet.</h2>
                <p class="date">Twins Robo / 3 Maret 2021</p>
              </div>
            </div>
          </a>
        </div>
      </div>

      <!-- Awal Pagination -->
      <ul class="pagination">
        <li class="page-item">
          <a class="page-link" href="#" aria-label="First">
            <span aria-hidden="true"> <img src="img/next.png" alt="" style="transform: rotate(180deg) scale(.6)"> </span>
          </a>
        </li>
        <li class="page-item">
          <a class="page-link" href="#" aria-label="Previous">
            <span aria-hidden="true"> <i class="bi bi-arrow-left"></i> </span>
          </a>
        </li>
        <li class="page-item"><a class="page-link" href="#">1</a></li>
        <li class="page-item"><a class="page-link" href="#">2</a></li>
        <li class="page-item"><a class="page-link" href="#">3</a></li>
        <li class="page-item">
          <a class="page-link" href="#" aria-label="Next">
            <span aria-hidden="true"> <i class="bi bi-arrow-right"></i> </span>
          </a>
        </li>
        <li class="page-item">
          <a class="page-link" href="#" aria-label="Last">
            <span aria-hidden="true"> <img src="img/next.png" alt="" style="transform: scale(.6);"> </span>
          </a>
        </li>
      </ul>
      <!-- Akhir Pagination -->
>>>>>>> 08363230f109d847aea069304296168905e1a1f6
    </div>
    <!--Akhir Blog Bagian Bawah -->
</div>
<!-- Akhir Blog -->

<!-- Awal Kategori Blog -->
<div id="kategoriArtikel">
    <!-- Awal Container -->
    <div class="container">
        <h1 class="aos-init aos-animate" data-aos="fade-right">Artikel Berdasarkan Kategori</h1>
        <div class="kategori">
            <div class="row aos-init aos-animate" data-aos="fade-up" data-aos-delay="200">
                <div class="col-lg-2 col-md-4 col-sm-6">
                    <a href="" class="h-100">
                        <img src="img/kategori1.png" alt="" />
                        <p>Program Robot</p>
                    </a>
                </div>
                <div class="col-lg-2 col-md-4 col-sm-6">
                    <a href="" class="h-100">
                        <img src="img/kategori2.png" class="bg-primary" alt="" />
                        <p>Mini Games</p>
                    </a>
                </div>
                <div class="col-lg-2 col-md-4 col-sm-6">
                    <a href="" class="h-100">
                        <img src="img/kategori3.png" class="bg-danger" alt="" />
                        <p>Info Lomba Robot</p>
                    </a>
                </div>
                <div class="col-lg-2 col-md-4 col-sm-6">
                    <a href="" class="h-100">
                        <img src="img/kategori4.png" class="bg-primary" alt="" />
                        <p>Learn Robot</p>
                    </a>
                </div>
                <div class="col-lg-2 col-md-4 col-sm-6">
                    <a href="" class="h-100">
                        <img src="img/kategori1.png" class="bg-primary" alt="" />
                        <p>Cek My Robot</p>
                    </a>
                </div>
                <div class="col-lg-2 col-md-4 col-sm-6">
                    <a href="" class="h-100">
                        <img src="img/kategori1.png" class="bg-primary" alt="" />
                        <p>Program Robot</p>
                    </a>
                </div>
                <div class="col-lg-2 col-md-4 col-sm-6">
                    <a href="" class="h-100">
                        <img src="img/kategori1.png" alt="" />
                        <p>Program Robot</p>
                    </a>
                </div>
                <div class="col-lg-2 col-md-4 col-sm-6">
                    <a href="" class="h-100">
                        <img src="img/kategori2.png" class="bg-primary" alt="" />
                        <p>Mini Games</p>
                    </a>
                </div>
                <div class="col-lg-2 col-md-4 col-sm-6">
                    <a href="" class="h-100">
                        <img src="img/kategori3.png" class="bg-danger" alt="" />
                        <p>Info Lomba Robot</p>
                    </a>
                </div>
                <div class="col-lg-2 col-md-4 col-sm-6">
                    <a href="" class="h-100">
                        <img src="img/kategori4.png" class="bg-primary" alt="" />
                        <p>Learn Robot</p>
                    </a>
                </div>
                <div class="col-lg-2 col-md-4 col-sm-6">
                    <a href="" class="h-100">
                        <img src="img/kategori1.png" class="bg-primary" alt="" />
                        <p>Cek My Robot</p>
                    </a>
                </div>
                <div class="col-lg-2 col-md-4 col-sm-6">
                    <a href="" class="h-100">
                        <img src="img/kategori1.png" class="bg-primary" alt="" />
                        <p>Program Robot</p>
                    </a>
                </div>
                <div class="col-lg-2 col-md-4 col-sm-6">
                    <a href="" class="h-100">
                        <img src="img/kategori1.png" class="bg-primary" alt="" />
                        <p>Cek My Robot</p>
                    </a>
                </div>
                <div class="col-lg-2 col-md-4 col-sm-6">
                    <a href="" class="h-100">
                        <img src="img/kategori1.png" class="bg-primary" alt="" />
                        <p>Program Robot</p>
                    </a>
                </div>
            </div>
            <!-- Akhir Row -->
        </div>
        <!-- Akhir Kategori -->
    </div>
    <!-- Akhir Container -->
</div>
<!-- Akhir Kategori Blog -->

<div id="labelBlog">
    <div class="container">
        <h1 class="aos-init aos-animate" data-aos="fade-right">Label</h1>
        <div class="row">
            <div class="col-lg-12 aos-init aos-animate" data-aos="fade-up">
                <div class="linkLabel">
                    <a href="">Rilis Berita</a>
                    <a href="">Jelajah</a>
                    <a href="">Data</a>
                    <a href="">Teknis</a>
                    <a href="">Tutorial</a>
                    <a href="">Ulasan</a>
                </div>
            </div>
        </div>
    </div>
</div>
<?= $this->endSection(); ?>